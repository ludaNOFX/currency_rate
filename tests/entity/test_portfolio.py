...
def test_initialization(initial_amounts, initial_currencies):
    portfolio = Portfolio(initial_amounts=initial_amounts, currencies=initial_currencies)
...
def test_get_rate(portfolio, exchange_rates):
    portfolio.exchange_rates = exchange_rates
...
def test_set_amount_one(portfolio):
...
def test_modify_amount_one(portfolio):
...
def test_set_multiple_amounts(portfolio):
...
def test_modify_multiple_amounts(portfolio):
...
def test_get_total(portfolio, exchange_rates):
    portfolio.exchange_rates = exchange_rates
    ...
    assert isinstance(total_usd, TotalCurrencyDTO)
...
def test_get_portfolio_summary(portfolio, exchange_rates):
    portfolio.exchange_rates = exchange_rates
    ...
    assert isinstance(summary.amounts, AmountCurrencyListDTO)
    assert isinstance(summary.rates, CurrencyListDTO)
    assert isinstance(summary.total, TotalCurrencyDTO)
...
def test_has_changes(portfolio, exchange_rates):
...
def test_update_rates(portfolio):
    ...
    assert portfolio.get_rate("RUB") == 105.0
    ...
    bad_rates = CurrencyListDTO(
        items=[CurrencyDTO(code="USD", value=-1.0), CurrencyDTO(code="EUR", value=105.0)]
    )
    with pytest.raises(PortfolioError):
        portfolio.update_rates(bad_rates)


def test_negative_amount_validation(initial_currencies):
